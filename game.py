...
class Game:
    ...
    def __init__(self, screen, server_ip, player):
        ...
        self.background_image = pygame.transform.scale(pygame.image.load(
            "data/Plansza.jpg"), (screen.get_width(), screen.get_height()))
        font_path = os.path.join(
            "data", "fonts", "Gothic_Ingame_Offiziell.ttf")  # Font z gothica
        self.font = pygame.font.Font(font_path, 70)
        self.font_comic = pygame.font.Font(
            os.path.join("data", "fonts", "comic.ttf"), 100)
        self.my_cards = []
        ...
        self.messageTimer = 0
        self.cardReverse = pygame.transform.scale(pygame.image.load(
            "data/textures/rewers.jpg"), (CARD_SIZE_X, CARD_SIZE_Y))
        self.cardToAttack = pygame.transform.scale(pygame.image.load(
            "data/textures/In Extremo.jpg"), (CARD_SIZE_X, CARD_SIZE_Y))
        ...
        rain_area = (int(screen.get_width() * 0.25),
                     int(space_for_line * 6),
                     int(screen.get_width()),
                     int(space_for_line * 6) + int(space_for_line))
        rain_area_opp = (int(screen.get_width() * 0.25),
                         int(space_for_line * 1),
                         int(screen.get_width()),
        ...
        snow_area = (int(screen.get_width() * 0.25),
                     int(space_for_line * 3),
                     int(screen.get_width()),
                     int(space_for_line * 3) + int(2 * space_for_line))
        self.graphicEffects.make_snow(snow_area, 250)
        fog_area = (int(screen.get_width() * 0.25),
                    int(space_for_line * 5),
                    int(screen.get_width()),
                    int(space_for_line * 5) + int(space_for_line))
        fog_area_opp = (int(screen.get_width() * 0.25),
                        int(space_for_line * 2),
                        int(screen.get_width()),
    ...
    def run(self):
        ...
        while running:
                    ...
                    if event.key == pygame.K_RIGHT:
                        ...
                        if self.choosing:
                            ...
                            if cardType == "back":
                                pos = "b"
                            mess = send_mess(
                                self, "M;" + str(self.cards_in_choose[self.pos_in_choose].id) + ";" + pos + ";\n")
            ...
            hero = pygame.transform.scale(
                self.heroCard.image, (CARD_SIZE_X, CARD_SIZE_Y))
            self.screen.blit(hero, (screen_width * 0.025, screen_height * 0.80,
                                    CARD_SIZE_X, CARD_SIZE_Y))

            hero = pygame.transform.scale(
                self.opp_heroCard.image, (CARD_SIZE_X, CARD_SIZE_Y))
            self.screen.blit(
                hero,
                (screen_width * 0.025,
                 screen_height * 0.025,
            ...
            pygame.draw.circle(self.screen,
                               color,
                               (screen_width * 0.025 + CARD_SIZE_X + 50,
                                screen_height * 0.025 + (CARD_SIZE_Y // 2)),
                               12)
            color = "white"
            ...
            pygame.draw.circle(self.screen,
                               color,
                               (screen_width * 0.025 + CARD_SIZE_X + 50,
                                screen_height * 0.8 + (CARD_SIZE_Y // 2)),
                               12)
            ...
            pygame.draw.rect(self.screen,
                             "black",
                             (0,
                              screen_height - (screen_height // 4) - screen_height * 0.2,
                                 screen_width * 0.2,
                                 screen_height * 0.2))  # bo rect.height = screen.height * 0.2

            text_surface = self.font.render("Przeciwnik", True, "white")
            text_rect = text_surface.get_rect(
                center=(screen_width * 0.1, (screen_height // 4) + (screen_height * 0.1)))
            self.screen.blit(text_surface, text_rect)

            text_surface = self.font.render("Ty", True, "white")
            text_rect = text_surface.get_rect(center=(
                screen_width * 0.1, screen_height - (screen_height // 4) - (screen_height * 0.1)))
            self.screen.blit(text_surface, text_rect)
            ...
            if self.opp_hp > 1:
                self.screen.blit(image,
                                 (0 + screen_width * 0.08,
                                  screen_height // 4 + (screen_height * 0.125)))
                self.screen.blit(image,
                                 (0 + screen_width * 0.12,
                                  screen_height // 4 + (screen_height * 0.125)))
            elif self.opp_hp > 0:
            ...
            pygame.draw.circle(self.screen,
                               "white",
                               (screen_width * 0.2,
                                screen_height // 4 + (screen_height * 0.1)),
                               30)
            if self.oppPassed:
            ...
            pygame.draw.circle(self.screen, "white", (screen_width * 0.2,
                               screen_height - (screen_height // 4) - (screen_height * 0.1)), 30)
            if self.passed:
                text_surface = self.font.render("PAS!", True, "red")
                text_rect = text_surface.get_rect(
            ...
            text_rect = text_surface.get_rect(
                center=(screen_width * 0.2, screen_height // 4 + (screen_height * 0.1)))
            self.screen.blit(text_surface, text_rect)

            text_surface = self.font.render(str(self.my_score), True, "Black")
            text_rect = text_surface.get_rect(center=(
                screen_width * 0.2, screen_height - (screen_height // 4) - (screen_height * 0.1)))
            self.screen.blit(text_surface, text_rect)

            # wyniki linie
            pygame.draw.circle(self.screen, "white", (screen_width * 0.26,
                               screen_height - (space_for_line * 2) + space_for_line // 2), 30)
            text_surface = self.font.render(
                self.myScores.split(";")[2], True, "red")
            text_rect = text_surface.get_rect(center=(
                screen_width * 0.26, screen_height - (space_for_line * 2) + space_for_line // 2))
            self.screen.blit(text_surface, text_rect)

            pygame.draw.circle(self.screen, "white", (screen_width * 0.26,
                               screen_height - (space_for_line * 3) + space_for_line // 2), 30)
            text_surface = self.font.render(
                self.myScores.split(";")[1], True, "green")
            text_rect = text_surface.get_rect(center=(
                screen_width * 0.26, screen_height - (space_for_line * 3) + space_for_line // 2))
            self.screen.blit(text_surface, text_rect)

            pygame.draw.circle(self.screen, "white", (screen_width * 0.26,
                               screen_height - (space_for_line * 4) + space_for_line // 2), 30)
            text_surface = self.font.render(
                self.myScores.split(";")[0], True, "Blue")
            text_rect = text_surface.get_rect(center=(
                screen_width * 0.26, screen_height - (space_for_line * 4) + space_for_line // 2))
            self.screen.blit(text_surface, text_rect)

            # =====================================

            pygame.draw.circle(self.screen, "white", (screen_width * 0.26,
                               screen_height - (space_for_line * 5) + space_for_line // 2), 30)
            text_surface = self.font.render(
                self.oppScores.split(";")[0], True, "Blue")
            text_rect = text_surface.get_rect(center=(
                screen_width * 0.26, screen_height - (space_for_line * 5) + space_for_line // 2))
            self.screen.blit(text_surface, text_rect)

            pygame.draw.circle(self.screen, "white", (screen_width * 0.26,
                               screen_height - (space_for_line * 6) + space_for_line // 2), 30)
            text_surface = self.font.render(
                self.oppScores.split(";")[1], True, "red")
            text_rect = text_surface.get_rect(center=(
                screen_width * 0.26, screen_height - (space_for_line * 6) + space_for_line // 2))
            self.screen.blit(text_surface, text_rect)

            pygame.draw.circle(self.screen, "white", (screen_width * 0.26,
                               screen_height - (space_for_line * 7) + space_for_line // 2), 30)
            text_surface = self.font.render(
                self.oppScores.split(";")[2], True, "green")
            text_rect = text_surface.get_rect(center=(
                screen_width * 0.26, screen_height - (space_for_line * 7) + space_for_line // 2))
            self.screen.blit(text_surface, text_rect)

            # karty
            for i in range(len(self.my_cards)):
                scaled_image = pygame.transform.scale(
                    self.my_cards[i].image, (CARD_SIZE_X, CARD_SIZE_Y))
                self.screen.blit(scaled_image,
                                 (screen_width * 0.26 + (i * CARD_SIZE_X * 0.9),
                                  screen_height - CARD_SIZE_Y))

            for i in range(self.opponent_cards_len):
                self.screen.blit(
                    self.cardReverse, (screen_width * 0.26 + (i * CARD_SIZE_X * 0.9), 0))

            # pole bitwy
            for i in range(len(self.MyFrontRow)):
                image = self.MyFrontRow[i].image
                image = pygame.transform.scale(
                    image, (CARD_SIZE_X, CARD_SIZE_Y))
                self.screen.blit(image,
                                 (screen_width * 0.36 + (i * CARD_SIZE_X * 0.9),
            ...
            if not self.stopHover:
                ...
                if self.card_to_display is not None:
                    for rect in self.rects_to_display:
                        ...
                        pygame.draw.rect(
                            surface, (255, 255, 0, 100), surface.get_rect())
                        self.screen.blit(surface, (rect[0], rect[1]))
                    self.screen.blit(
                        self.all_Cards[self.card_to_display].image, card_display_place)
                elif self.hero_card_to_display is not None:
                    self.screen.blit(
                        self.allHeroes[self.hero_card_to_display].image, card_display_place)
...
def falling_text(self, screen):
    if not (self.passed or self.endGame):
        if self.turn:
            ...
            text_surface = self.font_comic.render(
                "Ruch przeciwnika", True, "Red")
        text_rect = text_surface.get_rect(
            center=(
                self.falling_text_x,
                screen.get_height() // 2))
...
def checkIfMove(self):
    ...
    if self.hero_card_to_display is not None and self.isHeroActive.strip() == "T":
        received_message = send_mess(
            self, "H;" + str(self.hero_card_to_display) + ";" + "h" + ";\n")
...
def prepare_battlefield(self):
    ...
    self.opp_heroCard = self.allHeroes[int(
        send_mess(self, "GetOppHeroCard\n"))]
